...
// @Description Возвращает документ по указанному ID с учетом параметров запроса
// @Tags archive
// @Accept json
// @Produce json
// @Param id path string true "ID документа" format(uuid)
// @Param Host header string true "Хост запроса"
// @Param sender query string true "Отправитель запроса"
...
func (c *ArchiveController) GetDocument(ctx *gin.Context) error {
	...
	archives, err := c.service.GetDocument(ctx.Request.Context(), &req)
	if err != nil {
		return err
	}
